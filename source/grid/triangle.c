...
// To avoid conflicts with MPI, which also defines REAL, replace all usages
// below with double since deal.II always uses double for geometric types
...

{
  ...
  if ((b->voronoi || b->neighbors) &&
      (trisize < 6 * int(sizeof(triangle) + sizeof(int)))) {
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
               double *holelist, int holes, double *regionlist, int regions,
               int argc, char **argv)
#else /* not ANSI_DECLARATORS */
