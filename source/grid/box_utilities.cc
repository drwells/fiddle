...
namespace fdl
{
  ...
  /**
   * Another helper function since MultiblockPatchLevel and PatchLevel have slightly different interfaces
   */
  ...
  extract_patches(SAMRAI::tbox::Pointer<SAMRAI::hier::BasePatchLevel<spacedim>> base_patch_level)
  {
    ...
    tbox::Pointer<hier::MultiblockPatchLevel<spacedim>> multi_patch_level(base_patch_level);
    ...
    {
      for (unsigned int block_n = 0; block_n < multi_patch_level->getNumberOfBlocks(); ++block_n)
      {
  ...
  template
  std::vector<BoundingBox<NDIM, float>>
  compute_patch_bboxes(
    const std::vector<SAMRAI::tbox::Pointer<SAMRAI::hier::Patch<NDIM>>> &patches,
    const double extra_ghost_cell_fraction);

  template
  std::vector<BoundingBox<NDIM, double>>
  compute_patch_bboxes(
    const std::vector<SAMRAI::tbox::Pointer<SAMRAI::hier::Patch<NDIM>>> &patches,
    const double extra_ghost_cell_fraction);

  template
  std::vector<SAMRAI::tbox::Pointer<SAMRAI::hier::Patch<NDIM>>>
  extract_patches(SAMRAI::tbox::Pointer<SAMRAI::hier::BasePatchLevel<NDIM>> patch_level);

  template
  std::vector<SAMRAI::tbox::Pointer<SAMRAI::hier::Patch<NDIM>>>
  extract_patches(SAMRAI::tbox::Pointer<SAMRAI::hier::PatchLevel<NDIM>> patch_level);
} // namespace fdl
