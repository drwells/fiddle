        ...
        if (nodal_patch_map.size() == 0) {
            ...
            if (int_data)
                ...
                count_nodes_internal<dim, spacedim, double>(node_count_data_index,
                                                            nodal_patch_map,
                                                            position);
            else Assert(false, ExcFDLNotImplemented());
        }
    }
                    ...
                    switch (extract_depth(patch_data)) {
                        ...
                        case spacedim:
                            compute_spread_internal<dim,
                                    ...
                                    ARGUMENTS);
                            break;
                        default: AssertThrow(false, ExcNotImplemented());
                    }
                    break;
...
#undef ARGUMENTS
    ...
    if_line_intersect_with_box(const std::array<Point<3>, 3> &element_vertices, dealii::Point<3> &r,
        ...
        return (r[plane_axis[0]] >= x_min && r[plane_axis[0]] <= x_max && r[plane_axis[1]] <= y_max &&
                r[plane_axis[1]] >= y_min);
    }
    ...
    if_line_intersect_with_triangle(const std::array<Point<3>, 3> &element_vetices, dealii::Point<3> &r,
        ...
        int power = -7;
        if (abs(a) < pow(10, power) || abs(b) < pow(10, power) ||
            (abs(a + b) < (1 + pow(10, power)) && abs(a + b) > (1 - pow(10, power)))) {
                ...
                if (std::abs(a) > std::numeric_limits<double>::epsilon()) {
                    ...
                    if (u >= -tol && u <= 1.0 + tol) {
                        ...
                        if (v >= tol && (u + v) <= 1.0 + tol) {
                            double t = f * (e2 * cr);
                            t_vals.push_back(
                                    std::make_pair(t, Point<2>((1 - u - v) * ref_p0 + u * ref_p1 + v * ref_p2)));
                        }
                    }
                }
            }
