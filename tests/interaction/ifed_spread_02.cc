...
// More involved spreading test, where the force we compute is based on a finite
// element field which requires its own solution procedure.

using namespace dealii;
using namespace SAMRAI;

template <int dim, int spacedim = dim>
class SineForce : public fdl::ForceContribution<dim, spacedim>
{
public:
  SineForce()
    : fdl::ForceContribution<dim, spacedim>(QGauss<dim>(4))
  {}
...
protected:
  LinearAlgebra::distributed::Vector<double> sines;
};
