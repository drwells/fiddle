...
test(const Mapping<dim, spacedim>                     &mapping,
     ...
     const fdl::ForceContribution<dim, spacedim>      &f,
     ...
     std::ostream                                     &output)
{
  ...
  fdl::MechanicsValues<dim> me_values(fe_values,
                                      position,
                                      velocity,
                                      f.get_mechanics_update_flags());

  std::vector<Tensor<1, dim>> forces(quadrature.size());
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
      fe_values.reinit(cell);
      me_values.reinit(cell);

      auto view = make_array_view(forces.begin(), forces.end());
      f.compute_volume_force(0.0, me_values, cell, view);

      output << "cell = " << cell << " material id = " << cell->material_id()
             << std::endl;
      for (const auto &force : forces)
        output << "  " << force << std::endl;
    }
}
...
main()
{
  ...
  VectorTools::interpolate(dof_handler, Velocity<spacedim>(), velocity);
