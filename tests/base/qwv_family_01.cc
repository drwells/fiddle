...
{
  out << "dx = " << 0.1 << " DX = " << 0.05
      << " index = " << int(q_family.get_index(0.1, 0.05)) << '\n';
  out << "dx = " << 0.1 << " DX = " << 0.1
      << " index = " << int(q_family.get_index(0.1, 0.1)) << '\n';
  out << "dx = " << 0.1 << " DX = " << 0.2
      << " index = " << int(q_family.get_index(0.1, 0.2)) << '\n';
  out << "dx = " << 0.1 << " DX = " << 0.4
      << " index = " << int(q_family.get_index(0.1, 0.4)) << '\n';
  out << "dx = " << 0.1 << " DX = " << 0.8
      << " index = " << int(q_family.get_index(0.1, 0.8)) << '\n';
}
  ...
  for (unsigned int min_points_1D = 1; min_points_1D < 5; ++min_points_1D)
    {
      ...
      fdl::QWitherdenVincentSimplexFamily<2> q_family(min_points_1D);
