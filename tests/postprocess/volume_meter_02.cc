...
// Like volume_meter_01 but in 3D. The main point of this test is to check that
// changing to edge length vs. cell diameter results in less mesh refinement.
...
main(int argc, char **argv)
{
  ...
  auto tuple           = setup_hierarchy<3>(app_initializer);
  auto patch_hierarchy = std::get<0>(tuple);
  auto f_idx           = std::get<5>(tuple);

  test<3>(app_initializer, patch_hierarchy, f_idx, false);
  test<3>(app_initializer, patch_hierarchy, f_idx, true);
}
