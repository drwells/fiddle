...
main(int argc, char **argv)
{
  ...
  const auto partitioner = parallel::shared::Triangulation<2>::Settings::partition_zorder;
  ...
  if (argc > 1)
  {
    ...
    for (const char e : {'p', 'q'})
      for (const int i : {1, 2})
      {
        const std::string in_str = "." + std::string(std::size_t(1), e)
          + std::to_string(i) + ".";
        if (std::search(input_file.begin(), input_file.end(),
                        in_str.begin(), in_str.end()) != input_file.end())
        {
          degree = i;
  ...
  print_strings_on_0(local_out.str(),
                     comm,
                     output);
}
