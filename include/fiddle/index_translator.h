...
namespace fdl
{
  ...
  public:
    IndexTranslator(
      const std::vector<types::global_dof_index> &consecutive_dofs,
      const std::vector<types::global_dof_index> &nonconsecutive_dofs)
      : nonconsecutive_to_internal_index(
          nonconsecutive_dofs.size() == 0 ?
            0 :
                              ...
                              nonconsecutive_dofs.end()))
    {
      ...
      for (const auto &pair : temp)
        consecutive_to_non.push_back(pair.second);
      ...
      for (const auto &index : nonconsecutive_to_internal_index)
        {
          // Find the consecutive dof corresponding to the current
          // nonconsecutive dof.
          const auto it = std::lower_bound(
            ...
            [](const std::pair<types::global_dof_index, types::global_dof_index>
                 &                            a,
               const types::global_dof_index &b) { return a.second < b; });
    ...
    {
      return internal_to_consecutive[nonconsecutive_to_internal_index
                                       .index_within_set(b)];
    }
