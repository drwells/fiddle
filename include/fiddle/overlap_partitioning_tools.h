...
namespace fdl
{
    ...
    {
        const MPI_Comm mpi_comm = overlap_tria.get_communicator();
        ...
        for (const auto &cell : overlap_tria.active_cell_iterators())
        {
            ...
            native_active_cell_ids_on_overlap[subdomain_id]
        ...
        std::sort(overlap_dh_cells.begin(), overlap_dh_cells.end(),
        ...
        for (const auto &pair : native_dof_indices)
        {
            ...
            while (packed_ptr < native_dofs.cend())
            {
                ...
                // index.
                const auto overlap_dh_cell =
                    std::lower_bound(overlap_dh_cells.begin(),
                                     overlap_dh_cells.end(),
                                     active_cell_index,
                                     [&](const auto &a, const auto &val)
                                     {
                                         return overlap_tria.get_native_cell(a)
                                             ->active_cell_index() < val;
                                     });
        ...
        std::sort(overlap_to_native.begin(), overlap_to_native.end(),
        ...
        std::transform(overlap_to_native.begin(), overlap_to_native.end(),
